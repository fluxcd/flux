...
func updatePodController(def []byte, resourceID flux.ResourceID, container string, newImageID image.Ref) ([]byte, error) {
	fmt.Println("\n\t\t### ----------- updatePodController")
	fmt.Printf("\n\t\t\t\t container=%snewImageRef=%+v\n", container, newImageID)
...
func tryFHRUpdate(def []byte, container string, newImage image.Ref, out io.Writer) error {
	fmt.Println("=========================================")
	fmt.Println("\t\t*** in update.tryFHRUpdate")
