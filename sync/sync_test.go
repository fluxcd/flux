...
func TestSync(t *testing.T) {
	...
	if err := Sync(log.NewNopLogger(), manifests, resources, clus); err != nil {
		t.Fatal(err)
	}
...
func checkClusterMatchesFiles(t *testing.T, m cluster.Manifests, c Syncer, base string, dirs []string) {
	conf, err := c.Export()
	if err != nil {
		t.Fatal(err)
	}
